...
use psoc::file_io::{FileManager, is_supported_import_extension, can_export};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ✓ PNG loaded: {}x{} pixels", loaded_png.width(), loaded_png.height());
    
    println!("   Loading JPEG...");
    let loaded_jpg = file_manager.import_image(&jpg_path).await?;
    println!("   ✓ JPEG loaded: {}x{} pixels", loaded_jpg.width(), loaded_jpg.height());
    ...
    let jpeg_options = psoc::file_io::ExportOptions::Jpeg(
        psoc_file_formats::JpegOptions::high_quality()
    );
    ...
    let png_options = psoc::file_io::ExportOptions::Png(
        psoc_file_formats::PngOptions::default()
    );
    psoc::file_io::export_image_with_options(&dynamic_img, &png_options_path, png_options).await?;
    println!("   ✓ PNG with options saved: {}", png_options_path.display());
    
    // File size estimation
    println!("\n8. File Size Estimation:");
    let png_estimate = psoc::file_io::estimate_export_size(&dynamic_img, psoc_file_formats::SupportedFormat::Png);
    let jpeg_estimate = psoc::file_io::estimate_export_size(&dynamic_img, psoc_file_formats::SupportedFormat::Jpeg);
    ...
    println!("  All temporary files are in: {}", temp_dir.path().display());
